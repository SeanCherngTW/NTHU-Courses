...
void odd_even_sort(float*, int, int);
...
{   
    ...
    while(!sorted)
    {
        ...
        c2 = clock();
        commtime += c2-c1;
        odd_even_sort(p_nums, 0, p_element);
...
/* odd even sort in a processor */
void odd_even_sort(float* a, int index, int p)
{
    for (int i = index; i < p - 1; i += 2)
        if (a[i] > a[i + 1]) 
        { 
            swap(a[i], a[i + 1]); 
            sorted = false; 
